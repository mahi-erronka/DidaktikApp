...
@Component({
  ...
  templateUrl: './hizki-sopa.component.html',
  styleUrls: ['./hizki-sopa.component.scss'],
})
export class HizkiSopaComponent implements OnInit {
  private palabras = [
    'IBAIA',
    'JAIAK',
    'ANTZOKIA',
...
private Correct = new Audio('assets/audio/0Gunea/correctanswer.mp3');
private Incorrect = new Audio('assets/audio/0Gunea/wronganswer.mp3');
private Help = new Audio('assets/audio/0Gunea/help.mp3');
  ...
  marcarPalabraEncontrada(letrasSeleccionadas: { fila: number, columna: number }[]) {
    ...
    if (palabraEncontrada) {
      this.palabrasEncontradas.push({ palabra: palabraEncontrada, tachada: true });
      this.playAudio(this.Correct);
  ...
  actualizarPalabrasEncontradas() {
    const palabrasNoEncontradas = this.palabras.filter(
      (palabra) => !this.palabrasEncontradas.find(p => p.palabra === palabra)?.tachada
    );

    palabrasNoEncontradas.forEach((palabra) => {
      const palabraEncontrada = this.palabrasEncontradas.find(p => p.palabra === palabra);
      if (palabraEncontrada) {
    ...
    const palabrasEncontradasHTML = this.palabrasEncontradas
      .filter(p => p.tachada)
      .map(palabra => `<span class="palabra-encontrada">${palabra.palabra}</span>`);
